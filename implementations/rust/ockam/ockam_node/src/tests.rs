...
mod test {
    ...
    use ockam_core::{route, Result, Routed, Worker, Processor};
        ...
        for _ in 0..100 {
            ...
            executor
                .execute(async move {
                    ...
                    ctx.send(route!["child"], "Hello".to_string())
                        .await?;
    ...
    fn counting_processor__run_node_lifecycle__processor_lifecycle_should_be_full() {
        ...
        executor
            .execute(async move {
                ...
                ctx.start_processor("counting_processor", processor).await.unwrap();
    ...
    fn waiting_processor__shutdown__should_be_interrupted() {
        ...
        executor
            .execute(async move {
                ...
                ctx.start_processor("waiting_processor", processor).await.unwrap();
                sleep(Duration::new(1, 0)).await;
    ...
    fn waiting_processor__messaging__should_work() {
        ...
        executor
            .execute(async move {
                ...
                ctx.start_processor("messaging_processor", processor).await.unwrap();
                sleep(Duration::new(1, 0)).await;

                ctx.send(route!["messaging_processor"], "Keep working".to_string()).await.unwrap();
                assert_eq!("OK", ctx.receive::<String>().await.unwrap().take().body());

                assert!(initialize_was_called.load(Ordering::Relaxed));
                assert!(!shutdown_was_called.load(Ordering::Relaxed));

                ctx.send(route!["messaging_processor"], "Stop working".to_string()).await.unwrap();
                assert_eq!("I go home", ctx.receive::<String>().await.unwrap().take().body());
