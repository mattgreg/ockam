defmodule Ockam.TokenLeaseManager.StorageService.Postgres do
  @moduledoc false
  use Ockam.TokenLeaseManager.StorageService
  ...
  @impl true
  def handle_init({token_cloud_service, token_cloud_service_adress}) do
    opts =
      :ockam_hub
      |> Application.get_env(:postgres)
      |> Keyword.put(:token_cloud_service, token_cloud_service)
      |> Keyword.put(:token_cloud_service_adress, token_cloud_service_adress)
  ...
  def handle_save(
        %{
          ...
          token_cloud_service_adress: token_cloud_service_adress
        },
        lease
      ) do
    query =
  ...
  def handle_get(
    ...
    case Postgrex.query(conn, query, [lease_id, token_cloud_service, token_cloud_service_adress]) do
      {:ok,
       %Postgrex.Result{
         rows: [[^lease_id, ^token_cloud_service, ^token_cloud_service_adress, encoded_lease]]
       }} ->
  ...
  def handle_get_all(%{
    ...
    case Postgrex.query(conn, query, [token_cloud_service, token_cloud_service_adress]) do
      {:ok, %Postgrex.Result{rows: leases}} ->
        {:ok,
         for [_id, ^token_cloud_service, ^token_cloud_service_adress, encoded_lease] <- leases do
  ...
  defp create_storage_conf(opts) do
    ...
    {token_cloud_service_adress, postres_opts} =
      Keyword.pop(postres_opts, :token_cloud_service_adress)
